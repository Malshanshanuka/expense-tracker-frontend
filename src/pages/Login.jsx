...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      success(`Welcome back, ${data.fullName}!`);
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2 bg-slate-950">
      <div className="hidden lg:flex relative flex-col justify-between p-12 overflow-hidden bg-slate-900/40 border-r border-slate-800/80">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-violet-600/20 rounded-full blur-3xl pointer-events-none animate-pulse-glow" />
        ...
        <div className="relative z-10 max-w-md space-y-6">
          ...
          <p className="text-slate-400 text-base leading-relaxed">
            Monitor real-time monthly spending, visualize category breakdowns, and export professional reports effortlessly.
          </p>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4">
            <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md">
              ...
              <p className="text-xs text-slate-400">Secure JWT Authentication</p>
            </div>
            <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md">
              <p className="text-2xl font-bold text-white mb-1">Real-time</p>
              <p className="text-xs text-slate-400">Analytics & PDF Reports</p>
            </div>
          </div>
        </div>
